-- ===========================================================================
-- FASE 9 · EMAILS — SCHEMA EJECUTADO
-- ===========================================================================
--
--   ██  E S T O   S Í   S E   A P L I C Ó  ██
--
-- Estado final de la base después de la entrega 3, verificado contra
-- information_schema, pg_policy y pg_class.relacl.
--
-- La propuesta queda como histórica en PHASE_9_EMAILS_PROPOSAL.sql. Donde este
-- archivo difiera de aquélla, manda éste.
--
-- Migraciones aplicadas, en orden:
--   fase9_emails_entrega3_tablas
--   fase9_emails_entrega3_integridad
--   fase9_emails_entrega3_rls
--   fase9_emails_entrega3_rpc
--
-- La regla que ordena todo: el correo es de Gmail; el trabajo sobre el correo
-- es nuestro. Por eso NO hay cuerpos, no hay adjuntos binarios y no hay tabla
-- de mensajes.
--
-- Backend: Google Cloud Run con service account ADJUNTA, DWD vía
-- iamcredentials.signJwt. NINGUNA private key JSON.
--
-- Documentación de Google consultada el 2026-09-12.
-- ===========================================================================
...
-- ---------------------------------------------------------------------------
-- 10 · Las acciones del servidor — EJECUTADAS
-- ---------------------------------------------------------------------------
-- Sólo del backend (service_role); el frontend no las llama nunca:
--
--   tomar_lease_email(uuid, text, int)      reclama el lease en UNA sentencia
--   soltar_lease_email(uuid, text)          y sólo si sigue siendo suyo
--   avanzar_history_email(uuid, text, bool) el cursor, que NUNCA retrocede
--
-- Del ERP (authenticated, SECURITY DEFINER, validan al actor adentro):
--
--   asignar_hilo_email(uuid, text, uuid)
--   cambiar_estado_email(uuid, text, text)
--   vincular_cliente_email(uuid, text, uuid, uuid, text)
--
-- Del ERP, SECURITY INVOKER porque la regla ya está en la policy:
--
--   marcar_hilo_leido_email(uuid, text)
--   no_leidos_email(uuid, text[])
--
-- Las tres primeras crean la fila de estado si no existía: un hilo sin fila es
-- 'pendiente', así que la primera acción de una persona es la que la crea.
--
-- Y todo lo que habla con Gmail NO es una RPC de Postgres sino una ruta del
-- backend de Cloud Run, porque necesita la identidad de la service account:
--
--   POST /gmail/push    Pub/Sub, con OIDC validado en el código además de IAM
--   POST /gmail/watch   Cloud Scheduler, renovación diaria
--   POST /gmail/sync    recuperación manual
--   GET  /salud
--
-- /gmail/thread y /gmail/attachment llegan en la entrega 4, con la bandeja.
...
-- ===========================================================================
--
--   · La fila de email_accounts para info@buscatools.com.ar. No se crea hasta
--     que el backend esté desplegado: una cuenta que nadie sincroniza es peor
--     que ninguna.
--   · El cron de retención de email_sync_log (30 días). Documentado, no creado.
--   · Los endpoints /gmail/thread y /gmail/attachment: entrega 4.
--   · Envío, borradores y etiquetas: entrega 5.
--
-- ===========================================================================
-- FIN DEL SCHEMA EJECUTADO
-- ===========================================================================
