/**
 * Fase 8 · WhatsApp — genera los bloques de tipos de sus seis tablas.
 *
 ...
 * oficial.
 *
 * Emite los bloques `Tables` de las seis tablas de WhatsApp, listos para
 * pegar en `src/types/database.types.ts`. Con
 ...
 *
 *   · El nombre de la FK: `<tabla>_<columnas>_fkey`.
 *   · `isOneToOne: false` en todas: ninguna FK de WhatsApp tiene un índice
 *     único sobre exactamente sus columnas.
 *
 *   set -a; source .env; source .env.migration; set +a
 *   node scripts/fase8-generar-tipos-whatsapp.mjs            # imprime
 *   node scripts/fase8-generar-tipos-whatsapp.mjs --escribir # parchea
 */
...
function tipoTs(prop) {
  ...
  // Nada más aparece en estas seis tablas. Si aparece, mejor romper que
...
const main = async () => {
  ...
  for (const [tabla, bloque] of bloques) {
    ...
    let donde
    if (siguiente) {
      pos = siguiente.index + 1
      donde = `antes de ${siguiente[1]}`
    } else {
      // `whatsapp_*` va después de todo lo que existe: no hay «siguiente», así
      // que el lugar es el final del bloque Tables, justo antes de Views.
      const fin = archivo.indexOf('\n    Views: {')
      if (fin < 0) throw new Error(`No encontré el final del bloque Tables para ${tabla}`)
      const cierre = archivo.lastIndexOf('\n    }\n', fin)
      if (cierre < 0) throw new Error(`No encontré el cierre de Tables para ${tabla}`)
      pos = cierre + 1
      donde = 'al final de Tables'
    }
    archivo = archivo.slice(0, pos) + bloque + '\n' + archivo.slice(pos)
    insertados += 1
    console.log(`  insertada: ${tabla} (${donde})`)
  }
