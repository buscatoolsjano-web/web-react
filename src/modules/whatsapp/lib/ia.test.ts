...
describe('textos', () => {
  ...
  it('los errores del análisis se traducen sin jerga del proveedor', () => {
    ...
    expect(textoDeErrorIA('proveedor_auth')).toMatch(/credencial/)
    expect(textoDeErrorIA('proveedor_timeout')).toMatch(/tardó demasiado/)
    expect(textoDeErrorIA('proveedor_red')).toMatch(/conectar/)
    expect(textoDeErrorIA('proveedor_modelo_no_disponible')).toMatch(/no está disponible/)
    expect(textoDeErrorIA('proveedor_truncado')).toMatch(/incompleta/)
  })
})
